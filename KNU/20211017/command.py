...
class Expression:
    def __init__(self, expressionString=None, expressionId=None, expressionType=STRING):
        ...
        self.OR_parameters = None       # classifier learned from i/o examples
        self.cost = 1000                # final cost after training (if trained, cost is typically <10
...
'''
create a deep copy, where expression IDs are replaced with expression objects
    selections: list of ORed expressions in tree, which will be used to generate distinct codes
        in this list, childrens are to the left of their parents,
        as we add expressions by doing DFS and storing an expression if all its children are added
'''
selections = list()
...
'''
connect output to OR expressions that are currently selected
    output: list of outputs, one list for each input example
'''
def connectOutputsToOR(output):
    count = 0
    for orExp in selections:
        if isSelected(orExp):
            #print(orExp.toString() + '\n')
            if len(orExp.OR_output_list) == 0:
                for _ in range(len(orExp.OR)):
                    orExp.OR_output_list.append(list())
            #orExp.OR_output_list[orExp.selection].append(output)
            orExp.OR_output_list[orExp.currentSelection()].append(output)
            count += 1
    #print(str(count) + ' OR-expressions updated\n')
...
'''
reset selections in OR expressions to initial values
so that we can re-start from the first possible program

call this function before calling generateNextDistinctCode()
'''
def initializeCodeGenerationSequence():
    global distinctCodeCount
    distinctCodeCount = 0
    for orExp in selections:
        orExp.selection = 0
        if (len(orExp.selection_order) == 0):
            for i in range(len(orExp.OR)):
                orExp.selection_order.append(i)

        # by default, choose each OR option in a random order
        #random.shuffle(orExp.selection_order)
...
def generate(e):    # generate code according to current selections for ORed expressions
    if e.type == STRING:
        ...
        for esub in e.AND:
            ...
            # if temp has multiple lines, apply the indent of the first line to all following lines
            count = temp.count('\n')    # count # of linebreaks
            if temp[-1] == '\n':        # do not count the linebreak at the end, as no string follows it
                count = count - 1
            if count > 0:               # add indent after each linebreak, except the last                
                indent = (len(total) - total.rfind('\n') - 1) * ' ' # indent to the last '\n'
                temp = temp.replace('\n','\n' + indent, count)
        ...
        return generate(e.OR[e.currentSelection()])    
...
def isSelected(e):  # return whether e and all of its parents are currently selected
    while e.parent != None:
        ...
        if e.parent.type == ORED and e.parent.OR[e.parent.currentSelection()] != e:
            return False
        e = e.parent
